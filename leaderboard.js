...
export function renderLBU() {
  ...
  const arr = Object.entries(map)
    .map(([k, v]) => ({ k, u: UNITS[k], avg: v.count ? v.total / v.count : 0, total: v.total, count: v.count }))
  ...
  el.innerHTML = arr.map((x, i) => {
    ...
    // Ecossistema como pill colorido ao lado do nome
    const ecoTag = x.u?.eco
      ...
      : "";
    return `<div class="leaderboard__row" style="border-left:3px solid ${x.u?.color || "#888"}">
      <div class="leaderboard__rank ${RC(i)}">${RI(i)}</div>
      <div class="leaderboard__info">
...
// ── Ranking de uma Unidade específica ─────────────────────────────────────────
window.showUR = function (k, btn) {
  document.querySelectorAll(".utb").forEach(b => b.classList.remove("is-active"));
  ...
  el.innerHTML = pl.map((u2, i) => {
    ...
    return `<div class="leaderboard__row${me ? " leaderboard__row--me" : ""}">
      <div class="leaderboard__rank ${RC(i)}">${RI(i)}</div>
      <div class="leaderboard__avatar">${u2.emoji || "⚽"}</div>
      <div class="leaderboard__info">
        <div class="leaderboard__name" title="${u2.name}">${fmtName(u2.name)}${me ? ` <span style="color:var(--gold);font-size:.68rem">(${youText})</span>` : ""}</div>
      </div>
      <div class="leaderboard__points-col">
        <div class="leaderboard__points">${u2.pts || 0}</div>
        <div class="leaderboard__points-label">${getTranslation("pts_label")}</div>
      </div>
    </div>`;
  }).join("");
...
// ── Filtros de Unidade (botões "Por Unidade") ─────────────────────────────────
